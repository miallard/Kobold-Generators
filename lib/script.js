...
var koboldName = document.getElementById('name'),
...
function appearanceGenerator() {
    //gender generator (Male or Female)
...
function generateCommand(){
    var genShout = Math.random()*shouts.length;
        genShout = Math.trunc(genShout);
        genTarget = Math.random()*targets.length;
        genTarget = Math.trunc(genTarget);

    tacticsText.innerText = shouts[genShout] + targets[genTarget].toLowerCase() + "!";    
}  